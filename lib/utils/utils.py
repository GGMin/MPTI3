...
def get_batch_label(d, i, dataset):
    label = []

    #print(i)
    for idx in i:
        # label.append(list(d.labels[idx].values())[0])
        # print(list(d.labels[idx].values()))
        if dataset=="pretrain":
            x=d.labels[idx]
        elif dataset=="icdar2015":
            x=list(d.labels[idx].values())[0]
        elif dataset=="mysynthtext":
            x=idx
        #print(x)
        temp=[]
        for j in x:
            ...
            for chars in item:
                for char in chars:
                    ...
                    try:
                        ...
                        index = self.dict['?']
                        #print(char)
                        #print("a unknown char has showed up")
                    result.append(index)
                    #print(result)
        text = result
        #print(text)
